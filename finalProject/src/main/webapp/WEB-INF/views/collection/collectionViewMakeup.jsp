		...
		<div class="collectionViewBox">
			...
			<div class="view-gallery-slide">
				...
				<ul class="view-big-gallery-list slider-for">
					<li><img src="/resources/makeup/${makeup.makeupFilepath }"></li>
					<c:forEach items="${galleryList }" var="g">
						<li><img src="/resources/makeup/${g.filepath }"></li>
					</c:forEach>
				</ul>
				<!-- 작은 이미지 영역 -->
			...
			<div class="collectionDetailBox" style="position: relative;">
				...
				<div style="width: 70%; display: inline-block;">
					...
					<div style="text-align: center;">
						<span style="color:orange;font-size: 80px;vertical-align: middle;margin-right:30px;">${makeup.makeupScope/2}</span>
						<img src="/resources/img/scope-star/scope-star${makeup.makeupScope}.png" style="vertical-align: middle;">
					</div>
					<hr>
							...
							<div class="reviewGroup" id="${r.reviewNo}">
								...
								<div class="printReviewGroup" style="width:90%;display: inline-block;">
									...
									<c:if test="${sessionScope.member.memberName == r.reviewWriter}">
										<span style="margin-left:10px;" class="reviewUpdateGroup"><a class="updateReview" style="cursor: pointer;">수정</a> | <a class="deleteReview" style="cursor: pointer;">삭제</a><span class="makeupScopeSpan" style="cursor: inherit;width:10px;visibility: hidden;">${r.reviewScope}</span></span>
									</c:if>
					...
					<form id="fileUploadForm" action="/fileUpload.do" method="post" enctype="multipart/form-data">
						<div style="visibility: hidden;position: absolute;top:0;">
							<input type="text" id="code" name="code" value="M" style="cursor: inherit;width:10px;" readonly>
...
<script>
	...
	$(document).on("click",".updateSubmit",function(){
		...
		var objectNo = ${makeup.makeupNo};
		var code = "M";
		var reviewScope = parseInt($(this).siblings(".reviewUpdateGroup").find('.makeupScopeSpan').text());
	...
	$(document).on("click",".deleteReview",function(){
		...
		var objectNo = ${makeup.makeupNo};
		var code = "M";
		var reviewScope = $(this).siblings('.makeupScopeSpan').text();
	...
	$(document).on("click",".scrapStar",function(){
		...
		var objectNo = select.attr('id');		/* 업체 또는 상품 번호 */
		var code = select.attr('name');			/* 업체 또는 상품 타입분류 */
		var scrapbook = parseInt($('#scrapbookCount'));
